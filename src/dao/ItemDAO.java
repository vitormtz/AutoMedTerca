...
public class ItemDAO implements IDAOT<Item> {
    ...
    public void popularTabela(JTable tabela, String criterio) {
        ...
        try {
            resultadoQ = ConexaoBD.getInstance().getConnection().createStatement().executeQuery(""
                    + "SELECT i.id, i.descricao, a.descricao as descricao_apresentacao "
                    + "FROM item i, apresentacao a "
                    + "WHERE "
                    + "i.DESCRICAO ILIKE '%" + criterio + "%' AND "
                    + "i.apresentacao_id = a.id");

            while (resultadoQ.next()) {

                dadosTabela[lin][0] = resultadoQ.getInt("id");
                dadosTabela[lin][1] = resultadoQ.getString("descricao");
                dadosTabela[lin][2] = resultadoQ.getString("descricao_apresentacao");
